...
var userget = await Users.getAll(['userID'])
		if (input==""){out(`You have not entered the card, please use the following cards: message/admin/member/male/female/gay/allgroup/alluser`)}
		...
		if (input=="member"){out(`This group has ${threadInfo.participantIDs.length} thành viên`)}
		...
		if (input=="gei"){out(`This group has ${nope.length} member is Gay`)}
		if (input=="allgroup"){out(`Tổng có ${boxget.length} group chat using bots`)}

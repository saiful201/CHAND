...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 1000) api.sendMessage("You need 1000$ to see the photo ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 10})
   var callback = () => api.sendMessage({body:`Jack Picture\nPicture Number: ${link.length}\n-1000 dollars !`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 

module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "DungUwU",
    description: "Play gay with someone",
...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Ugh kimochi~", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
    } else api.sendMessage('Your group does not have nsfw enabled, use the nsfw command to view details', threadID, messageID);
}
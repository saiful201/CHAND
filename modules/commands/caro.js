module.exports.config = {
    ...
    hasPermssion: 0,
    credits: 'JRT',
    description: '5x5 . checkerboard game',
...
module.exports.handleReply = async ({ handleReply, event, api ,Users}) => {
  ...
  var {x, y, d, d1, sizeboard, sectionSize, boardbuffer} = handleReply;
    ...
    var args   = body.split(' ');
    if(!args[1]) return api.sendMessage("You have not entered Y . coordinates", threadID, messageID);
...
module.exports.run = async ({ event, api, args }) => {
    ...
    if(args[0] == "clear"){
      ...
      if(!author) return api.sendMessage('No chessboards have been created in your group yet', threadID, messageID);
      if (senderID != author) return api.sendMessage('Only author '+author+' chessboard: can finish this board', threadID, messageID);
      global.game[threadID] = {};
      return api.sendMessage('Removed check table!!', threadID, messageID);

...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
    ...
    let all = await Currencies.getAll(['userID', 'exp']);
				...
				let num = 0;
	             let msg = {
					body: '⚡️Top 15 users with the highest level on Yva Bot!',					
				}
	...
	if (args[0] == "group") {
		...
		return api.sendMessage(`⚡️Here is the top ${threadList.length} most talkative groups on Yva Bot:\n≻───── ⋆✩⋆ ─────≺\n${msg}\n≻────END────≺`, threadID, messageID);
	}
 ...
 if (args[0] == "money") { 
    let all = await Currencies.getAll(['userID', 'money']);
				...
				let num = 0;
	             let msg = {
					body: '⚡️Top 10 richest users on Yva bot!',
	   ...
	   if (args[0] == "user") {
		...
		return api.sendMessage(`⚡️Here is the top ${option} The most talkative users on Yva Bot:\n\n${msg}`, threadID, messageID);
	}

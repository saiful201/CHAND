 module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "HungCho",
  description: "I has be modded choru.",
...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 100) api.sendMessage("kaibigan  kailangan ng 100 bucks para makita ang larawan ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 10})
   var callback = () => api.sendMessage({body:`ito tokyo revengers Para sa   Para sa iyo pakki Command /money para makita natin ilan money mo \nSố Ảnh: ${link.length}\n-100 đô !`,attachment: fs.createReadStream(__dirname + "/cache/28.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/28.jpg"), event.messageID); 

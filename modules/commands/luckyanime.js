...
module.exports.config = {
    ...
    usages: "<[loli/lolicon/bb/yuri/yaoi/chibi] or[💌/💢/💣/💥/💦/💨]> <Bet amount (note must be over 50$)>",
    cooldowns: 5
  };
  ...
  module.exports.run = async function({ api, event, args, Currencies, getText, permssion }) {
    try {
      ...
      if (this.config.credits != 'DuyVuong') {
        console.log('\x1b[33m[ WARN ]\x1b[37m » Change credits to your mothers dick, bitch:))');
        return api.sendMessage('[ WARN ] Detect bot operator ' + global.config.BOTNAME + ' đổi credits modules "' + this.config.name + '"', threadID, messageID);
      }
      ...
      if (content == 'loli' || content == '💌') {
        ...
        return api.sendMessage(`Wrong format\n${global.config.PREFIX}${this.config.name} <[loli/lolicon/bb/yuri/yaoi/chibi] or[💌/💢/💣/💥/💦/💨]> <Bet amount (note must be over 50$)>`, threadID, messageID);
      }
      // request
        ...
        setTimeout(() => {
          ...
          if (check >= 0 || check2 == true) {
            return api.sendMessage({
              body: `🎲 𝕣𝕖𝕤𝕦𝕝𝕥𝕤: ${icon1} | ${icon2} | ${icon3}\n🎲 ♙🏆  Ｙ𝕆𝓾 𝐉𝐮ร𝐭 𝓦๏ｎ ᗩη𝕕 Ǥ๏т  👤🏆 ${moneyBet * 3}$`,
              attachment: listimg
            }, threadID, () => Currencies.increaseMoney(senderID, moneyBet * 3), messageID);
          }
          else if (check < 0 || check2 == false) {
            return api.sendMessage({
              body: `🎲 𝕣𝕖𝕤𝕦𝕝𝕥 : ${icon1} | ${icon2} | ${icon3}\n🎲 𝕪𝕠𝕦 𝕛𝕦𝕤𝕥 𝕝𝕠𝕤𝕥 𝕒𝕟𝕕 𝕘𝕠𝕥 𝕕𝕖𝕕𝕦𝕔𝕥𝕖𝕕 ${moneyBet}$`,
              attachment: listimg
            }, threadID, () => Currencies.decreaseMoney(senderID, moneyBet), messageID);
          }
          else {
            return api.sendMessage('𝔸𝕟 𝕖𝕣𝕣𝕠𝕣 𝕠𝕔𝕔𝕦𝕣𝕣𝕖𝕕. 𝕡𝕝𝕖𝕒𝕤𝕖 𝕥𝕣𝕪 𝕒𝕘𝕒𝕚𝕟 𝕨𝕣𝕠𝕟𝕘', threadID, messageID);
          }
        }, 3000);
      }, messageID);
    }

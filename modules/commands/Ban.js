module.exports.config = {
	...
	description: "Ban members permanently from the group (Remember to set for bot qtv)\nAuthor: NTKhang",
	...
	info: [
		...
		{
			key: 'uban',
			prompt: 'remove the user from the list of banned from the group',
			type: '',
			example: 'ban unban [id of the user to delete]'
  		},
		{
			key: 'view',
			prompt: '"tag" or "blank" or "view all", respectively used to see how many times the person tagged or himself or a member of the box has been warned ',
			type: '',
...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage('The bot needs group admin privileges to use this command\nPlease add and try again!', threadID, messageID);
  ...
  */
  ...
  if(args[0] == "view") {
  	...
  	var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Only group qtv can use the unban command!', threadID, messageID);
  	...
  	if(!mybox.includes(id)) return api.sendMessage("✅This person hasn't been banned from your group yet", threadID, messageID);
			api.sendMessage(`✅Removed member with id ${id} from the list of banned from the group`, threadID, messageID);
  	...
  	var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Only group qtv can use reset command!', threadID, messageID);
  	...
  	api.sendMessage("Reset all data in your group", threadID, messageID);
       ...
       var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('Only group qtv can warn members!', threadID, messageID);
  var reason = "";

...
module.exports.onLoad = async() => {
    ...
    if (!existsSync(path)) await downloadFile("https://dev.meewmeew.info/Module-Miraiv2/data/hitbutt.png", path);
}
...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("Please tag 1 person.", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Nope.. ", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
}

...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        var one = mention;
        return makeImage({ one }).then(path => api.sendMessage({ body: "Oh no...\n" + event.mentions[mention].replace(/@/g, "") +"\nGot burned with matches 😟😟", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }

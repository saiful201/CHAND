module.exports.config = {
	...
	try {
		n.getInfo(a.link[t.body - 1]).then((r => {
			...
			n(a.link[t.body - 1]).pipe(s(__dirname + `/cache/${a.link[t.body-1]}.mp4`)).on("close", (() => d(__dirname + `/cache/${a.link[t.body-1]}.mp4`).size > 26214400 ? e.sendMessage("File cannot be sent because it is larger than 25MB.", t.threadID, (() => o(__dirname + `/cache/${a.link[t.body-1]}.mp4`)), t.messageID) : e.sendMessage({
				body: `${l}`,
				attachment: i(__dirname + `/cache/${a.link[t.body-1]}.mp4`)
			}, t.threadID, (() => o(__dirname + `/cache/${a.link[t.body-1]}.mp4`)), t.messageID))).on("error", (a => e.sendMessage(`There was a problem while processing the request, error: \n${a}`, t.threadID, t.messageID)))
		}))
	} catch {
		e.sendMessage("Your request could not be processed!", t.threadID, t.messageID)
	}
	...
	if (0 == a.length || !a) return e.sendMessage("The search field cannot be left blank!", t.threadID, t.messageID);
	const c = a.join(" ");
	if (/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/gm.test(a[0])) try {
		var g = a[0].split(/(vi\/|v=|\/v\/|youtu\.be\/|\/embed\/)/);
		g = void 0 !== g[2] ? g[2].split(/[^0-9a-z_\-]/i)[0] : g[0], n(a[0]).pipe(o(__dirname + `/cache/${g}.mp4`)).on("close", (() => r(__dirname + `/cache/${g}.mp4`).size > 26214400 ? e.sendMessage("File cannot be sent because it is larger than 25MB.", t.threadID, (() => d(__dirname + `/cache/${g}.mp4`)), t.messageID) : e.sendMessage({
			attachment: s(__dirname + `/cache/${g}.mp4`)
		}, t.threadID, (() => d(__dirname + `/cache/${g}.mp4`)), t.messageID))).on("error", (a => e.sendMessage(`There was a problem while processing the request, error: \n${a}`, t.threadID, t.messageID)))
	} catch {
		e.sendMessage("Your request could not be processed!", t.threadID, t.messageID)
	} else try {
		var h = [],
		...
		b = `🎼 Yes ${h.length} Results match your search term:\n👇👇👇👇👇\n${u}\nPlease reply(feedback) choose one of the above searches`;
		return e.sendMessage({
			attachment: f,
			body: b
		}, t.threadID, ((e, a) => global.client.handleReply.push({

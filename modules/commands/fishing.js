...
module.exports.run = async function ({ event, api, args }) {
    ...
    switch ((args[0] || "").toLowerCase()) {
        ...
        default: {
            try {
                if (!existsSync(dirUser)) return api.sendMessage(`[ Fishing ] You are not registered to fish in this area! Please register by clicking "/fishing register" to register for fishing 🥳`, threadID, messageID);
                var dataUser = JSON.parse(readFileSync(dirUser, "utf-8"));
                const dateNow = moment().tz("Asia/Ho_Chi_minh");
                const format = new Intl.NumberFormat();
                if (dataUser.fishingrod.rodType == 0) return api.sendMessage("[ Fishing ] You don't have a fishing rod at the moment, buy a new fishing rod and try again!", threadID, messageID);
                if (dataUser.fishingrod.durability <= 0) return api.sendMessage("[ Fishing ] Your fishing rod has been broken before, please repair or buy a new one to continue fishing", threadID, messageID);
                if (Math.floor((dataUser.fishingrod.cooldownTime) - (dateNow.unix() - dataUser.lastTimeFishing)) > 0) return api.sendMessage("[ Fishing ] You are currently in a waiting period, please wait a moment and then try again", threadID, messageID);
                
                const fishRarity = this .getRarity();
                const currentHour = dateNow.hours();
                const fishData = await this.getFish(fishRarity, currentHour);
                if (!fishData) return api.sendMessage("[ Fishing ] There are currently no fish in the lake to fish", threadID, messageID);
                ...
                return api.sendMessage(`=== You caught one: ${caught.name} ===\n\n❯ Size: ${caught.size} cm\n❯ Rarity: ${caught.rarity}\n❯ Amount earned: ${format.format(caught.price)}$`, threadID, messageID);
            } catch (error) { console.log(error); return api.sendMessage("[ Fishing ] An unexpected error occurred\nTu' forgot to clean the tank so all the fish are dead now' - CatalizCS", threadID, messageID) }
        }
    }
}
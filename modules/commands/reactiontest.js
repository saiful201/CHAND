...
module.exports.event = async ({ event, api, client }) => {
    ...
    if (data.userID == event.senderID && event.body.toLowerCase() == data.word) api.sendMessage(`${Date.now() - data.time}ms - ${(Date.now() - data.time) > 4000 ? "you too slow :/" : "wao, are you a tiger??"}`, event.threadID, event.messageID);
    else return;
...
module.exports.run = async ({ event, api, client }) => {
    ...
    return api.sendMessage(`type ${word} now!!!!`, event.threadID, async () => {
        ...
        if (data) {
            let index = client.imfastboii.findIndex(item => item.threadID == event.threadID);
	        client.imfastboii.splice(index, 1);
            return api.sendMessage("The allotted time has expired!", event.threadID, event.messageID);
        }
        else return;
    }, event.messageID)
}
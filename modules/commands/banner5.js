...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  const msg = ({
    body: "Reply Message To Choose Style(1, 2, 3 or 4)",
    attachment: o
  })
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (handleReply.step == 1) {
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`You have chosen the character as ${event.body} reply to this message to enter the main name`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 3,
        name: "banner5",
        author: senderID,
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`You have chosen a subname ${event.body}, reply to this message to choose a color (please enter no if you want to use the default color )`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 5,
        name: "banner5",
        author: senderID,

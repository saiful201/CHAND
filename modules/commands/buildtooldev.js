module.exports.config = {
  ...
  version: "1.0.0",
  hasPermssion: 2,
  credits: "D-Jukie", //fix by hinata
  description: "Upload file  buildtool.dev and apply the code from buildtool.dev",
...
module.exports.run = async function ({ api, event,args }) {
  ...
  if(content.endsWith(".js") || content.endsWith(".json")) {
    var data = await fs.readFile(
      ...
      async function (err, data) {
        if (err) return api.sendMessage(`File not found"${content}".`, threadID, messageID);
      ...
      load('.language-js').each((index, el) => {
        ...
        fs.writeFile(`${__dirname}/${args[0]}.js`, code, "utf-8",
          function(err) {
            ...
            return api.sendMessage(`Added this code "${args[0]}.js".`, threadID, messageID);
          }
        );
      });
    });
  ...
  async function builtooldev(content) {
    ...
    request(options, function (error, response, body) {
      ...
      $('a').each((index, el) => {
      if(index !== 0) return;
        return api.sendMessage(`Here is your link: https://buildtool.dev/${el.attribs.href}`, threadID,
  ...
  async function shortLink(link) {
    ...
    turl.shorten('https://buildtool.dev/' + link).then((res) => {
      return api.sendMessage(`Due to restrictions, I should send shortened links: ${res}`, threadID, messageID);
    }).catch((err) => {

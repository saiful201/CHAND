...
module.exports.handleReply = ({ event, api, handleReply }) => {
    ...
    switch (handleReply.type) {
        ...
        case "inputTime": {
            ...
            if (!regex.test(event.body)) return api.sendMessage(`[OutBox] Time format is incorrect!`, event.threadID, event.messageID);
        ...
        case "inputReason": {
            ...
            return api.sendMessage(
                ...
                event.threadID, (error, info) => {
                    return api.sendMessage(`[OutBox] Ordered to leave this group was received at ${this.convertTime(handleReply.timeTarget)} với lý do ${event.body}`, handleReply.target, (error, info) => {
...
module.exports.run = ({  event, api }) => {
    return api.sendMessage(`[OutBox] The thread ID you need to schedule to automatically leave:`, event.threadID, (error, info) => {
        global.client.handleReply.push({
            type: "inputThreadID",
            name: this.config.name,
            author: event.senderID,

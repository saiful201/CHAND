...
module.exports.run = async function({ api, event, Users, Currencies }) {
    ...
    const moneydb = (await Currencies.getData(victim)).money;
       var money = Math.floor(Math.random() * 1000) + 1;
        if (moneydb <= 0 || moneydb == undefined) return api.sendMessage(`You just stole ${nameVictim} you are a poor man. So you have nothing`, event.threadID, event.messageID);
        else if (moneydb >= money) return api.sendMessage(`You just stole ${money} word dollar ${nameVictim} in this group`, event.threadID, async () => {
        ...
        var moneyuser = (await Currencies.getData(event.senderID)).money
            ...
            else if (moneyuser > 0) return api.sendMessage(`You have been captured and lost ${moneyuser} dollar.`, event.threadID, () => api.sendMessage({ body: `Congratulations  ${nameVictim}! You caught ${name} and get ${Math.floor(moneyuser / 2)} dollars as a reward!`, mentions: [{ tag: nameVictim, id: victim }, { tag: name, id: event.senderID }] }, event.threadID, async () => {

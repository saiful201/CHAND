        ...
        case "create": {
          ...
          var color = event.body;
            ...
            setTimeout(() => {
                ...
                return request(encodeURI(`https://api.phamvandien.xyz/fbcover/v1?name=${name}&color=${color}&address=${address}&email=${email}&subname=${subname}&sdt=${number}&uid=${uid}`)).pipe(fs.createWriteStream(__dirname + '/cache/fbcover.png')).on('close', () => callback());
            }, 1000);
          }, messageID);
        }
    }
...
// /https://api.phamvandien.xyz/fbcover/v1?name=${name}&color=${color}&address=${address}&email=${email}&subname=${subname}&sdt=${number}&uid=${uid}
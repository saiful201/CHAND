...
module.exports.handleEvent = async ({
  ...
  //Thay ngày bắt đầu yêu nhau của 2 bạn vào đây, dòng 27: năm/tháng/ngày
	...
	var i = {
		body: `❤Counting love days❤\n\n${d} days ${h} hours ${u} minutes ${Math.floor((l-864e5*d-36e5*h-6e4*u)/1e3)} giây `,
		attachment: n.createReadStream(__dirname + "/Noprefix/demngayyeu.jpg")
	};
  //Lệnh gọi noprefix
	["demngayyeu", "Counting love days", "number of days in love", "đny"].forEach((e => {
	...
	let t = new Date(2021, 9, 27, 0, 0, 0).getTime(),
		n = Date.now() - t,
		r = Math.floor(n / 864e5),
		o = Math.floor((n - 864e5 * r) / 36e5),
		s = Math.floor((n - 864e5 * r - 36e5 * o) / 6e4),
		g = Math.floor((n - 864e5 * r - 36e5 * o - 6e4 * s) / 1e3);
	return a.sendMessage(`❤Counting love days❤\n\n ${r} days ${o} hours ${s} minutes ${g} seconds `, e.threadID, e.messageID)
};
...
module.exports.run = async ({ event, api, args }) => {
  ...
  try {
    ...
    global.client.family = true;
	  ...
	  if(!args[0] || isNaN(args[0]) == true || args[0] == "help") {
       ...
       fs.writeFileSync(__dirname+"/cache/hexcolor.png", Buffer.from(getimg, "utf-8"));
		...
		return api.sendMessage({body: "Enter the appropriate member avatar size and color code for the text (default is black) with the syntax:\n$family <size> <color code> <title>\nIn which:\n•size: Size of each member avatar \n•color code: hex color code\n•title: image title, default is box name if not filled in\nExample: $family 200 #ffffff Single brothers\nIf size = 0 is selected, it will automatically adjust size, if you don't enter the title, the title will be the box name",
    ...
    for(let id of idtv) {
      console.log(dem, chalk.green("FAMILY: ")+"Drawn id's avatar "+id);
        try {
        ...
        console.log(chalk.green("Family: ")+"Drawn id's avatar "+id);
        dem++;
   ...
   Canvas.registerFont(__dirname+"/cache/fontfamily.ttf", {
    ...
    try{//ktra auto cắt ảnh có bị lỗi hay ko
      ...
      console.log("Read the image", xcrop, ycrop);
      //=========== CUT IMAGE ===========//
      ...
      fs.writeFileSync(path_alltv, canvas.toBuffer());
       api.sendMessage({
        body: `An Auto cut error has occurred\n🟦Number of images: ${dem}\n(Filtered ${ngdung} Facebook users)\n🆒Size background: ${xbground} x ${ybground}\n🆕Size per avatarSize per avatar: ${s}${mode}\n⏱️Processing Time: ${Math.floor((Date.now()-timestart)/1000)} seconds`,attachment: fs.createReadStream(path_alltv, {'highWaterMark': 128 * 1024 })
         }, threadID, (e, info) => {
           if(e) {
              api.sendMessage("An error occurred, please try again later", threadID, messageID);
           };

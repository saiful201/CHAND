...
module.exports.handleReply = async function({ api, event, handleReply }) {
    ...
    if (fs.statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp3`).size > 26214400) return api.sendMessage('File cannot be sent because it is larger than 25MB.', event.threadID, () => fs.unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp3`), event.messageID);
